import React, { useState } from 'react';
import { Head, Link } from '@inertiajs/react';
import NavbarV3 from './Components/NavbarV3';
import FooterV3 from './Components/FooterV3';
import CartDrawer from '@/Components/CartDrawer';
import SectionBoxV3 from './Components/SectionBoxV3';
import { ChevronRight, Search } from 'lucide-react';

export default function BrandsV3({
  brands = [],
  featuredBrands = [],
  settings = {},
}) {
  const [cartOpen, setCartOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');

  const fallbackBrands = [
    { id: 'b1', name: 'Xiaomi', slug: 'xiaomi' },
    { id: 'b2', name: 'WEIDASI', slug: 'weidasi' },
    { id: 'b3', name: 'UNIKYY', slug: 'unikyy' },
    { id: 'b4', name: 'Transcend', slug: 'transcend' },
    { id: 'b5', name: 'SOLOVE', slug: 'solove' },
    { id: 'b6', name: 'SKE', slug: 'ske' },
    { id: 'b7', name: 'SITECOM', slug: 'sitecom' },
    { id: 'b8', name: 'SATECHI', slug: 'satechi' },
    { id: 'b9', name: 'SAMSUNG', slug: 'samsung' },
    { id: 'b10', name: 'RAZER', slug: 'razer' },
    { id: 'b11', name: 'QCY', slug: 'qcy' },
    { id: 'b12', name: 'PROMATE', slug: 'promate' },
    { id: 'b13', name: 'msi', slug: 'msi' },
    { id: 'b14', name: 'Microsoft', slug: 'microsoft' },
    { id: 'b15', name: 'Lenovo', slug: 'lenovo' },
    { id: 'b16', name: 'JYSUPER', slug: 'jysuper' },
    { id: 'b17', name: 'IRIScan', slug: 'iriscan' },
    { id: 'b18', name: 'hp', slug: 'hp' },
    { id: 'b19', name: 'Hollyland', slug: 'hollyland' },
    { id: 'b20', name: 'hama', slug: 'hama' },
    { id: 'b21', name: 'GIGABYTE', slug: 'gigabyte' },
    { id: 'b22', name: 'COUGAR', slug: 'cougar' },
    { id: 'b23', name: 'brother', slug: 'brother' },
    { id: 'b24', name: 'BOSE', slug: 'bose' },
    { id: 'b25', name: 'BenQ', slug: 'benq' },
    { id: 'b26', name: 'Awei', slug: 'awei' },
    { id: 'b27', name: 'ASUS', slug: 'asus' },
    { id: 'b28', name: 'Apple', slug: 'apple' },
    { id: 'b29', name: 'ALOGIC', slug: 'alogic' },
    { id: 'b30', name: 'acer', slug: 'acer' },
  ];

  const brandList = (brands && brands.length > 0) ? brands : fallbackBrands;

  const filtered = brandList.filter(b =>
    b.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <div className="storefront-v3 min-h-screen bg-[#F4F7FC] text-slate-900 font-sans flex flex-col selection:bg-[#0153FD] selection:text-white">
      <Head title={`Official Brands - ${settings.site_name || 'TechMarket BD'}`} />

      {/* 1. Navbar */}
      <NavbarV3 onOpenCart={() => setCartOpen(true)} />
      <CartDrawer isOpen={cartOpen} onClose={() => setCartOpen(false)} />

      {/* 2. Breadcrumb */}
      <div className="w-full bg-white border-b border-slate-100 py-2 px-4 sm:px-6 lg:px-8">
        <div className="max-w-[1240px] mx-auto flex items-center space-x-2 text-xs text-slate-500 font-medium">
          <Link href="/" className="hover:text-[#0153FD] transition-colors">Home</Link>
          <ChevronRight className="w-3.5 h-3.5 text-slate-400" />
          <span className="text-slate-900 font-semibold">Brands</span>
        </div>
      </div>

      {/* 3. Main Brand Showcase Layout (Screenshot 1) */}
      <main className="flex-1 max-w-[1240px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        
        {/* Search Input for Brands */}
        <div className="max-w-md mx-auto mb-8 relative">
          <input
            type="text"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Search brands by name..."
            className="w-full bg-white text-xs sm:text-sm text-slate-800 placeholder-slate-400 pl-4 pr-10 py-2.5 rounded-full border border-[#8BB1FF]/70 shadow-xs focus:border-[#0153FD] focus:outline-none"
          />
          <Search className="w-4 h-4 text-slate-400 absolute right-3.5 top-3" />
        </div>

        {/* Signature TechJhuli Section Box for Brands */}
        <SectionBoxV3 title="Brands" badgeText="Brands">
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 sm:gap-4 pt-2">
            {filtered.map((brand) => (
              <Link
                key={brand.id}
                href={`/catalog?brand=${brand.slug || brand.name.toLowerCase()}`}
                className="bg-[#fbfcfe] hover:bg-white border border-slate-200/80 hover:border-[#0153FD] rounded-xl sm:rounded-2xl p-3 sm:p-4 h-16 sm:h-20 flex items-center justify-center text-center transition-all duration-200 hover:shadow-sm group cursor-pointer"
              >
                {brand.logo ? (
                  <img
                    src={brand.logo}
                    alt={brand.name}
                    className="max-h-full max-w-full object-contain grayscale group-hover:grayscale-0 transition-all"
                    loading="lazy"
                  />
                ) : (
                  <span className="font-bold text-xs sm:text-sm text-slate-700 group-hover:text-[#0153FD] tracking-wide transition-colors truncate">
                    {brand.name}
                  </span>
                )}
              </Link>
            ))}
          </div>
        </SectionBoxV3>
      </main>

      {/* 4. Footer */}
      <FooterV3 onOpenCart={() => setCartOpen(true)} />
    </div>
  );
}
