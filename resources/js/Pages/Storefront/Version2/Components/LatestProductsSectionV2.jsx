...
export default function LatestProductsSectionV2({ products = [] }) {
  ...
  return (
    <div className="storefront-v2-latest-products w-full max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8 select-none">
      {/* Section Header */}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        {visibleProducts.map((product) => (
          <ProductCardV2 key={`latest-${product.id}`} product={product} />
        ))}
      </div>
    </div>
  );
